...
import io.factorialsystems.msscpirateparrotauthorization.repository.AuthorizationRepository;
...
public class JpaOAuth2AuthorizationService implements OAuth2AuthorizationService {
    ...
    public void save(OAuth2Authorization authorization) {
        ...
        if (authorizationRepository.IsExist(authorization.getId())) {
            log.info("Updating Authorization {}", entity.getId());
            this.authorizationRepository.update(entity);
        } else {
            log.info("Saving Authorization {}", entity.getId());
    ...
    public void remove(OAuth2Authorization authorization) {
        log.info("Deleting Authorization {}", authorization.getId());
    ...
    public OAuth2Authorization findByToken(String token, OAuth2TokenType tokenType) {
        ...
        if (tokenType == null) {
            ...
            log.info("Finding Authorization By Token_CODE: {}", token);
            result = this.authorizationRepository.findByAuthorizationCodeValue(token);
        } else if (OAuth2ParameterNames.ACCESS_TOKEN.equals(tokenType.getValue())) {
            log.info("Finding Authorization By Token_ACCESS_TOKEN: {}", token);
            result = this.authorizationRepository.findByAccessTokenValue(token);
        } else if (OAuth2ParameterNames.REFRESH_TOKEN.equals(tokenType.getValue())) {
            log.info("Finding Authorization By Token_REFRESH_TOKEN: {}", token);
            result = this.authorizationRepository.findByRefreshTokenValue(token);
        } else {
            log.info("Finding Authorization By Token_UNKNOWN: {} TokenType: {}", token, tokenType.toString());
    ...
    private OAuth2Authorization toObject(Authorization entity) {
        ...
        if (registeredClient == null) {
            throw new DataRetrievalFailureException(
                    "The UserRegisteredClient with id '" + entity.getRegisteredClientId() + "' was not found in the RegisteredClientRepository.");
        }

        OAuth2Authorization.Builder builder = OAuth2Authorization.withRegisteredClient(registeredClient)
                .id(entity.getId())
                .principalName(entity.getPrincipalName())
                .authorizationGrantType(ResolveAuthorizationGrantType.resolve(entity.getAuthorizationGrantType()))
